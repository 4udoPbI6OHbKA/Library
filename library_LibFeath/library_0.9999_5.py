...
class LibraryApp:
    def __init__(self, root):
        ...
        # Создаем Frame для output_widget
        self.output_frame = tk.Frame(self.root)
        self.output_frame.place(relx=0.3, rely=0.28, relwidth=0.4, relheight=0.3)
        self.output_background = set_background(self.output_frame, 'foninf.jpg')

        self.output_widget = tk.Text(self.output_frame, wrap=tk.WORD, state="normal", background=None) # Убираем background у текста
        self.output_widget.place(relx=0, rely=0, relwidth=1, relheight=1)

        scrollbar = tk.Scrollbar(self.output_frame, command=self.output_widget.yview) # привязываем скроллбар к фрейму, чтобы правильно отображался
        ...
        self.output_widget.insert(tk.END, "Версия библиотеки 0.9999_1\n")
    ...
    def open_library(self):
        ...
        self.readbook_button = create_button(self.buttons_frame, "Читать книгу", self.open_book, 0, 1, width=20, height=2)
    ...
    def open_book(self):
        ...
        books_path = filedialog.askdirectory(title="Выберите папку с книгами")
        ...
        if files is None or not files:
            self.output_widget.config(state="normal")
            self.output_widget.insert(tk.END, "В выбранной папке нет текстовых файлов (.txt).\n")
            self.output_widget.see(tk.END)
            self.output_widget.config(state="disabled")
            return
