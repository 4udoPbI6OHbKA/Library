...
class LibraryApp:
    def __init__(self, root):
        ...
        scrollbar.place(relx=0.9, rely=0.1, relheight=0.4)
        ...
        self.entry_frame = tk.Frame(self.root)  # Frame for entry fields
        self.entry_frame.place(relx=0.1, rely=0.52, relwidth=0.8, relheight=0.2)
        self.entry_frame.columnconfigure(1, weight=1)

        # Entry Fields and Labels (Initially Hidden)
        ...
        self.library_buttons_created = False #Флаг для отслеживания, были ли созданы кнопки библиотеки
    ...
    def show_registration_fields(self):
        self.hide_entry_fields()  # Clear any previous fields
    ...
    def logout(self):
        ...
        self.show_login_register_buttons() #Показываем кнопки регистрации/логина/выхода

        self.output_widget.config(state="disabled")
        self.clear_entries()
        self.hide_entry_fields() #Hide fields when logging out

    def show_login_register_buttons(self):
        # Уничтожаем кнопки библиотеки, если они существуют
        if self.booklist_button:
            self.booklist_button.destroy()
            self.booklist_button = None
        if self.readbook_button:
            self.readbook_button.destroy()
            self.readbook_button = None
        if self.back_button:
            self.back_button.destroy()
            self.back_button = None

        # Показываем кнопки логина, регистрации и выхода
        self.reg_button.grid(row=0, column=0)
        self.login_button.grid(row=0, column=1)
        self.exit_button.grid(row=0, column=2)

        # Уничтожаем кнопки "Библиотека", "Выход из учетной записи", "Выход"
        ...
        if self.exit_button:
            ...
            self.exit_button = None #Добавлено уничтожение кнопки exit

        
        self.library_buttons_created = False #Сбрасываем флаг

    def show_library_buttons(self):
        #Скрываем кнопки логина, регистрации и выхода
        if self.reg_button:
        ...
        self.hide_entry_fields() #Hide fields on successful login
    ...
    def open_library(self):
        ...
        self.back_button = create_button(self.buttons_frame, "Назад", self.show_login_register_buttons, 0, 2) #Вызываем функцию для показа кнопок логина/регистрации
