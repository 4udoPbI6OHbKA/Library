...
def create_button(parent, text, command, row, column, image_path="кнопка.png", padx=5, pady=5, sticky=tk.W+tk.E+tk.N+tk.S, width=None, height=None, font=('Arial', 10), compound=tk.LEFT):
    image = Image.open(image_path)
...
class LibraryApp:
    def __init__(self, root):
        ...
        # Добавляем Frame для фона информационного окна
        self.output_frame = tk.Frame(self.root)
        self.output_frame.place(relx=0.3, rely=0.28, relwidth=0.4, relheight=0.3)

        # Устанавливаем фон для Frame
        self.output_background = set_background(self.output_frame, 'fon_info.jpg')  # Замените 'output_fon.jpg' на путь к вашей картинке

        self.output_widget = tk.Text(self.output_frame, wrap=tk.WORD, state="normal", background=None) # Убираем фон текста
        self.output_widget.place(relx=0.3, rely=0.28, relwidth=0.4, relheight=0.3) # Заполняем Frame
        ...
        self.output_widget.insert(tk.END, "Версия библиотеки 0.9999\n")
        ...
        self.buttons_frame.place(relx=0.1, rely=0.64, relwidth=0.8, relheight=0.05)  
