...
def create_button(parent, text, command, row, column, image_path="button.png", padx=5, pady=5, sticky=tk.W+tk.E+tk.N+tk.S, width=None, height=None, font=('Arial', 10), compound=tk.CENTER):
    image = Image.open(image_path)
    ...
    btn.grid(row=row, column=column, padx=padx, pady=pady, sticky=sticky, sizefrom=None)
    return btn
...
class LibraryApp:
    def __init__(self, root):
        ...
        self.output_widget.place(relx=0.3, rely=0.28, relwidth=0.4, relheight=0.3)


        scrollbar = tk.Scrollbar(self.root, command=self.output_widget.yview)
        scrollbar.place(relx=0.7, rely=0.28, relheight=0.29)
        self.output_widget['yscrollcommand'] = scrollbar.set

        self.output_widget.insert(tk.END, "Добро пожаловать в библиотеку LibFeath\n")
        self.output_widget.insert(tk.END, "Версия библиотеки 1.0\n")
        ...
        self.entry_frame.place(relx=0.1, rely=0.57, relwidth=0.8, relheight=0.2)
        self.entry_frame.columnconfigure(1, weight=1)
        self.entry_background = set_background(self.entry_frame, 'fon_widget.jpg')
        ...
        self.book_number_label = tk.Label(self.entry_frame, text="Номер книги:")
        self.book_number_entry = tk.Entry(self.entry_frame)
        ...
        self.exit_button = create_button(self.buttons_frame, "Выход", self.exit_app, 0, 2, width=20, height=2)

        self.library_button = create_button(self.buttons_frame, "Библиотека", self.open_library, 0, 0, width=20, height=2)
        self.logout_button = create_button(self.buttons_frame, "Выход из учетной записи", self.logout, 0, 1, width=20, height=2)
    ...
    def hide_entry_fields(self):
        ...
        self.book_number_label.grid_forget()
        self.book_number_entry.grid_forget()
    ...
    def show_login_register_buttons(self):
        ...
        self.reg_button.grid(row=0, column=0, sticky=tk.W+tk.E+tk.N+tk.S)
        self.login_button.grid(row=0, column=1, sticky=tk.W+tk.E+tk.N+tk.S)
        self.exit_button.grid(row=0, column=2, sticky=tk.W+tk.E+tk.N+tk.S)
    ...
    def show_library_buttons(self):
        ...
        self.library_button.grid(row=0, column=0, sticky=tk.W+tk.E+tk.N+tk.S)
        self.logout_button.grid(row=0, column=1, sticky=tk.W+tk.E+tk.N+tk.S)
        self.exit_button.grid(row=0, column=2, sticky=tk.W+tk.E+tk.N+tk.S)
    ...
    def open_library(self):
        ...
        self.booklist_button = create_button(self.buttons_frame, "Весь список книг", self.show_booklist, 0, 0, width=20, height=2)
        self.readbook_button = create_button(self.buttons_frame, "Читать книгу", self.show_book_number_field, 0, 1, width=20, height=2)
        self.back_button = create_button(self.buttons_frame, "Назад", self.show_library_buttons, 0, 2, width=20, height=2)

        self.booklist_button.grid(row=0, column=0, sticky=tk.W+tk.E+tk.N+tk.S)
        self.readbook_button.grid(row=0, column=1, sticky=tk.W+tk.E+tk.N+tk.S)
        self.back_button.grid(row=0, column=2, sticky=tk.W+tk.E+tk.N+tk.S)
    ...
    def show_book_number_field(self):
        self.hide_entry_fields()
        self.book_number_label.grid(row=5, column=0, sticky=tk.W)
        self.book_number_entry.grid(row=5, column=1, sticky=tk.W + tk.E)


        def open_book_wrapper():
            self.open_book(self.book_number_entry.get())
        self.readbook_button.config(command=open_book_wrapper)
        self.back_button.config(command=self.hide_book_number_field)

    def hide_book_number_field(self):
        self.hide_entry_fields()
        self.readbook_button.config(command=self.show_book_number_field)
        self.back_button.config(command=self.show_library_buttons)
    ...
    def open_book(self, book_number_str):
        ...
        if files is None or not files:
            self.output_widget.config(state="normal")
            self.output_widget.insert(tk.END, "Кажется в библиотеке нет книг\n")
            self.output_widget.see(tk.END)
            self.output_widget.config(state="disabled")
            return
        ...
        try:
            ...
            if 1 <= book_number <= len(files):
                selected_file = os.path.join(books_path, files[book_number - 1])
                self.open_file(selected_file)
                self.output_widget.config(state="normal")
                self.output_widget.insert(tk.END, f"Открыта книга: {files[book_number - 1]}\n")
                self.output_widget.see(tk.END)
                self.output_widget.config(state="disabled")
            else:
                self.output_widget.config(state="normal")
                self.output_widget.insert(tk.END, "Неверный номер книги.\n")
                self.output_widget.see(tk.END)
                self.output_widget.config(state="disabled")
        except ValueError:
            self.output_widget.config(state="normal")
            self.output_widget.insert(tk.END, "Пожалуйста, введите число.\n")
            self.output_widget.see(tk.END)
            self.output_widget.config(state="disabled")
